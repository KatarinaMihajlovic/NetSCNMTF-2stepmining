...
def BarPlotValidGenes(PDpreds_enrich, file_out):
    ...
    plt.savefig(file_out,  dpi = 350, format='jpg', bbox_inches='tight')	
    plt.show()
    plt.close()
...
def PDGs2Background_dist(Predictions_pubmed, BG_pubmed, day, save_dir, set2_label = 'background'):
    ...
    ax.legend(handles= [Patch(facecolor='red', alpha = 0.5, edgecolor='red',label=f'(1) PD predictions ({len(Predictions_pubmed)})'),
                Patch(facecolor='cornflowerblue', alpha = 0.5, edgecolor='cornflowerblue',label=f'(2) {set2_label} ({len(BG_pubmed)})')],
              loc=1, labelspacing=1, prop={'size': 28}, facecolor ='white')
    
    if set2_label == 'background':
        savext = 'BG'
    elif set2_label == 'Stage Spec PD preds':
        savext = 'SSP'
    elif set2_label == 'LIGERIntersection':
        savext = 'CP'
    elif set2_label == 'Core PD predictions':
        savext = 'CP'

    if day != 'Core PD predictions':
        plt.savefig(f'{save_dir}/C{day}PD{day}_PDpredsvs{savext}_MWU.jpg', dpi = 350, format='jpg')        
    else:
        plt.savefig(f'{save_dir}/LIGERIntersection_PDpredsvs{savext}_MWU.jpg', dpi = 350, format='jpg')                    
    plt.show()
    plt.close()
...
preds_dir = 'LIGERpreds' 
PDpreds_enrich = []
if not os.path.exists(f'output/{preds_dir}'):
    os.makedirs(f'output/{preds_dir}')  
outf = open(f'output/LIGERpreds/{file_out}','w')


CGs_PD_preds_all = {}

for root, dirs, files in os.walk(f'input/{preds_dir}'):
    for file in files:
        if file.endswith('.csv'):
            filen = file.split('.')[0]
            print(filen)
            save_dir = f'output/{preds_dir}'
            day = filen.split('_')[2]
            day2 = day.split('D')[1]
            
            cc1cc2_PD_preds = pd.read_csv(f'{root}/{file}', header = 0)
            cc1cc2_PD_preds = cc1cc2_PD_preds['feature'].tolist()
            cc1cc2_PD_preds = [Entrez2Sym[str(x)] for x in cc1cc2_PD_preds]
            CGs_PD_preds_all[day] = cc1cc2_PD_preds
            ...
            with open(f'input/StageSpecPreds/C{day2}PD{day2}_PDpreds.pkl', 'rb') as handel:
                StageSpecPreds = pickle.load(handel)   
            StageSpecPreds_pubmed, _ = logPubmedn(StageSpecPreds)

            #check with background             
            save_dir2 = f'output/{preds_dir}/Other_genes'
            if not os.path.exists(save_dir2):
                os.makedirs(save_dir2)  
 
            with open(f'input/All_genes/C{day2}PD{day2}_All_genes.pkl', 'rb') as handel:
            ...
            with open(f'{save_dir2}/{day}_OGs_LitValid.pkl', 'wb') as fp:   
            ...
            # PlotHist_MWU(cc1cc2_pubmed, StageSpecPreds_pubmed, cc_pair, save_dir, label2='Stage Spec PD preds')
...
file_out = 'output/LIGERpreds/LitValid_Enrich.jpg'
BarPlotValidGenes(PDpreds_enrich, file_out)



######### Core PD predictions Validation
save_dir = 'output/LIGERIntersection'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)  
with open('input/LitValid_AllGenes.pkl', 'rb') as handle:
    LitValid_AllGenes = pickle.load(handle) 
...
CGs_PD_preds = set.intersection(*map(set,CGs_PD_preds_all.values()))
...
with open(f'{save_dir}/LIGERIntersection_LitValid.txt', 'w') as f:
...
PDGs2Background_dist(CGs_PD_preds_pubmed, OtherGenes_pubmed, 'LIGERIntersection', save_dir)
...
bar1 = ax.bar(np.arange(1), LitEnr_genes[1], width, align='center', alpha=opacity, color='r', label='LIGER intersection')
