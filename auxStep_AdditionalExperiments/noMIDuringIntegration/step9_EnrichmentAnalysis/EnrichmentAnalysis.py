...
# no MI during integration
with open('input/CorePreds_noMI.pkl', 'rb') as handle:
    PD_genes = pickle.load(handle)  
save_file = sd = 'CorePreds_noMI'
...
for term in KP_terms[0]:
    ...
    for gene, KP_terms_d in gene2go_kp.items():  
        if term in KP_terms_d and gene in PD_genes:
...
ax.set_title('Top most significantly enriched KEGG pathways', fontsize=34, fontweight = 'bold', pad=25, x=-0.3, y=1.02)
...
plt.savefig(f'output/{sd}/EnrichmentKEGG_hbar_CorePreds.jpg', dpi = 350, bbox_inches='tight', format='jpg')
plt.show()
plt.close()
