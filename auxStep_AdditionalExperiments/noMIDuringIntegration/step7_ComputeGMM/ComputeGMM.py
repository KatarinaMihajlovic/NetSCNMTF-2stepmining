...
cell_conds = list(set(cell_conds))
...
for C1_i in range(len(cell_conds)): 
    ...
    G1 = pd.read_csv(f'{base_dir}/{cell_cond_1}/PPI+GI+COEX_G1_with_headers.csv', header=0, index_col=0, delimiter='\t')
    S5 = pd.read_csv(f'{base_dir}/{cell_cond_1}/PPI+GI+COEX_S_EXP.csv', header=0, index_col=0, delimiter='\t')
